...
- For deterministic cancellation and controlled parallel-failure smoke: `CELERY_WORKER_CONCURRENCY=1`.
- For isolated integration cleanup coverage: set `UPLOAD_DIR=.integration-runtime/uploads`, `OUTPUT_DIR=.integration-runtime/outputs`, `TEMP_DIR=.integration-runtime/tmp`, and `INTEGRATION_WORK_DIR=.integration-runtime/work`.

Opt-in integration commands:

```bash
RUN_CJK_BURNIN_SMOKE=1 python -m pytest -q -m integration tests/test_cjk_burnin_integration.py -ra
INTEGRATION_TEST_MODE=true CELERY_WORKER_CONCURRENCY=1 python scripts/run_real_integration_smoke.py --base-url http://127.0.0.1:8891
```

Use `http://127.0.0.1:8891` for local process startup and `http://127.0.0.1:9091` for Docker-based startup. Do not mark either opt-in check passed unless the command actually completed successfully. Generated smoke videos land under `INTEGRATION_WORK_DIR`; task artifacts land under `UPLOAD_DIR`, `OUTPUT_DIR`, and `TEMP_DIR`; keep backend, worker, beat, and smoke logs for failed runs. Remove `.integration-runtime` after the run if you want to clean the integration workspace.
...
| 7. Segment failure handling | Run the real smoke script with `INTEGRATION_TEST_MODE=true`, or trigger the maintained deterministic segment-failure case manually. | Original task is FAILURE and temp segments/locks are removed. | `/status/{task_id}`, lock files, temp segment listing. | Worker logs, `UPLOAD_DIR/{task_id}_error.json`. |
| 8. Normal task cancellation | Start a task with worker concurrency `1`, click cancel, or `curl -X POST http://127.0.0.1:8891/tasks/{task_id}/cancel`. | Task becomes CANCELED and remains canceled on repeated polling. | Status responses before and after cancel. | Backend logs, worker logs. |
...
| 17. Protected downloads | Set `REQUIRE_AUTH_TOKEN=true` and `AUTH_TOKEN`, rebuild frontend with `VITE_API_TOKEN`, then download files. | Direct unauthenticated download is 401; authenticated ticketed final-video, ASS, SRT, VTT, and batch ZIP downloads succeed; invalid or expired tickets are rejected. | Unauthorized response, `/download-ticket` response, ticketed URL response. | Backend logs, browser network tab. |
...
| 22. Expired-file cleanup preserves active tasks | Create old artifacts plus an active lock, then invoke the real registered cleanup task through Celery or wait for beat to dispatch it. | Expired unlocked files are removed; active task artifacts remain. | Directory listing before/after, cleanup counts. | Worker logs, backend cleanup logs. |
